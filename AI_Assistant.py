...
def is_tshark_installed():
    ...
    try:
        #--These 2 lines allows for tshark connection to work on windows machines when tshark is in the PATH---
...
def execute_live_packet_capture(interface: str = None, packet_count: int = 10) -> str:
    ...
    try:
        ...
        Tshark_path = r"C:\Program Files\Wireshark\tshark.exe" #--- This line below is better for running on Windows machine---
        cmd = [
            #---
            #"tshark", "-i", interface, ##---This works on Mac OS---
            Tshark_path, "-i", interface, ###--- This is the Tshark_path in use for Windows---
            "-T", "fields",
            "-e", "frame.time",
            "-e", "ip.src",
            "-e", "ip.dst",
...
def researcher_node(state: AgentState):
    ...
    if selected_researcher:
        ...
        if "live traffic" in user_request.lower() or "capture" in user_request.lower() or "packet" in user_request.lower():
            ...
            prompt = f"Answer this cybersecurity question directly (max 200 words): {user_request}"
